...
<span typeof="v:Breadcrumb"><strong class="breadcrumb_last"
                                    property="v:title"><?php post_type_archive_title(); ?></strong></span>
</span>
            </div>
			<?php
		else :
...
function dwqa_search_form() {
    ...
    <form id="dwqa-search" class="dwqa-search">
        <input data-nonce="<?php echo wp_create_nonce( '_dwqa_filter_nonce' ) ?>" type="text"
               placeholder="<?php _e( 'What do you want to know?', 'dwqa' ); ?>" name="qs"
               value="<?php echo isset( $_GET['qs'] ) ? esc_html( $_GET['qs'] ) : '' ?>">
    </form>
	<?php
}
	...
	if ( comments_open( $post_id ) ) :
        ...
        <div id="dwqa-respond" class="dwqa-comment-form">
			<?php if ( ! dwqa_current_user_can( 'post_comment' ) ) : ?>
				...
				?>
			<?php else : ?>
                <form method="post" id="<?php echo esc_attr( $args['id_form'] ); ?>"
					...
					?>
                    <input name="comment-submit" type="submit" id="<?php echo esc_attr( $args['id_submit'] ); ?>"
                           value="<?php echo esc_attr( $args['label_submit'] ); ?>" class="dwqa-btn dwqa-btn-primary"/>
